...
class MagicBall extends Module {
    ...
    reply(message: Message, replyContent: string) {
    ...
    loadResponse() {
        const responsesUrl = String(process.env.MBALL_RESPONSE);
        return httpUtils.get(responsesUrl).then(res => res.data);
    }

    loadCustomResponse() {
        const customResponsesUrl = String(process.env.MBALL_CUSTOM_RESPONSE);
